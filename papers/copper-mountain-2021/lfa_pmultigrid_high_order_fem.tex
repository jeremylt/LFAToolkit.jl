...
We investigate the performance of Jacobi and Chebyshev smoothing for p-multigrid with aggressive coarsening for the scalar Laplacian in one and two dimensions and validate this LFA formulation with the scalar Laplacian in three dimensions.
...
\Cref{sec:results} contains numerical results investigating the performance of Jacobi smoothing for p-multigrid on the one, two, and three dimensional scalar Laplacian and three dimensional linear elasticity.
...
To demonstrate the capability of this LFA formulation for more complex PDE, we consider linear elasticity in three dimensions.
...
\end{equation}
for some $\mathbf{u} \in \mathcal{V} \subset H^1 \left( \Omega \right)$, where $:$ denotes contraction over both indices.

In the linear elasticity constitutive model, the symmetric strain tensor is given by
\begin{equation}
\boldsymbol{\epsilon} = \frac{1}{2} \left( \nabla \mathbf{u} + \nabla \mathbf{u}^T \right).
\end{equation}
The linear elasticity constitutive law is given by $\boldsymbol{\sigma} = \mathsf{C} : \boldsymbol{\epsilon}$ where
\begin{equation}
\mathsf{C} =
\begin{bmatrix}
