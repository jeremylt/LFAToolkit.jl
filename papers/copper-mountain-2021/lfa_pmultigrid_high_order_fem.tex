...
The basis $\mathbf{B}$ is defined on the mesh grid with points given by $x_i$, for $i \in \left\lbrace 0, 1, \dots, p \right\rbrace$, where $p$ is the polynomial order of the basis.
...
\end{equation}
where $\odot$ represents pointwise multiplication of the elements, $\mathbf{h}$ is the length of the element in each dimension, $\boldsymbol{\theta}$ is the target frequency in each dimension, and $i, j \in \lbrace 0, 1, \dots, n \cdot \left( p + 1 \right)^d - 1 \rbrace$, $n$ is the number of components, $p$ is the polynomial order of the discretization, and $d$ is the dimension of the finite element basis.
...
\end{equation}
where $i \in \lbrace 0, 1, \dots, n \cdot \left( p_{\text{fine}} + 1 \right)^d - 1 \rbrace$, $\mathbf{h}$ is the length of the element in each dimension, $j \in \lbrace 0, 1, \dots, n \cdot \left( p_{\text{coarse}} + 1 \right)^d - 1 \rbrace$, $n$ is the number of components, $p_{\text{fine}}$ and $p_{\text{coarse}}$ are the polynomial orders of the fine and coarse grid discretizations, respectively, and $d$ is the dimension of the finite element basis.
...
\end{equation}
where $i \in \lbrace 0, 1, \dots, n \cdot \left( p_{\text{coarse}} + 1 \right)^d - 1 \rbrace$, $\mathbf{h}$ is the length of the element in each dimension, $j \in \lbrace 0, 1, \dots, n \cdot \left( p_{\text{fine}} + 1 \right)^d - 1 \rbrace$, $n$ is the number of components, $p_{\text{fine}}$ and $p_{\text{coarse}}$ are the polynomial orders of the fine and coarse grid discretizations, respectively, and $d$ is the dimension of the finite element basis.
...
The element level operator, $\mathbf{A}_e$, on the fine grid macro-element assembles the element level operator for each micro-element into the action of the PDE operator on the full macro-element.
...
\tilde{\mathbf{R}}_{\text{ftoc}} \left( \boldsymbol{\theta} \right) = \mathbf{Q}_c^T \left( \mathbf{R}_e \odot \left[ e^{\imath \left( \mathbf{x}_{j, f} - \mathbf{x}_{i, c} \right) \cdot \boldsymbol{\theta} / \mathbf{h}} \right] \right) \mathbf{Q}_f
\end{equation}
where $i \in \left\lbrace 0, 1, \dots, n \left( p + 1 \right)^d - 1 \right\rbrace$, $j \in \left\lbrace 0, 1, \dots, n \left( m \left( p - 1 \right) + 1 \right)^d - 1 \right\rbrace$, $\mathbf{h}$ is the length of the macro-element, $d$ is the dimension of the finite element bases, $n$ is the number of components, $m$ is the number of micro-elements in each fine grid macro-element, $p$ is the polynomial order of the discretization, and $d$ is the dimension of the finite element basis.
