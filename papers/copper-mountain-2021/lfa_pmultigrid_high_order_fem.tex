...
Specifically,  we develop LFA of p-multigrid with arbitrary second-order PDEs using high-order finite element discretizations and examine the performance of Jacobi and Chebyshev smoothing for two-grid schemes with agressive p-coarsening.
A natural fruit of this LFA framework is the extension to h-multigrid with standard coarsening or aggressive coarsening.  Some examples from 1D to 3D are presented to validate our LFA prediction of p-multigrid.
\end{abstract}
% -----------------------------------------------------------------------------
...
Our LFA formulation for p-multigrid with high-order finite element discretizations for the continuous Galerkin method can be generalized to reproduce previous work on h-multigrid for high-order finite elements \cite{he2020two},  and can be extended to LFA for h-multigrid with aggressive coarsening.
...
\yh{remove this subsection to Numerical section?}
...
Boundary terms have been omitted, as they are not presented on the infinite uniform grid $G_h$.
...
The basis $\mathbf{B}$ is defined on the mesh grid with points given by $x_i$, for $i \in \left\lbrace 1, 2, \dots, p + 1 \right\rbrace$ in one element, where $p$ is the polynomial order of the basis.
The nodes on the left and right boundaries of the element map to the same Fourier mode due to periodicity, so we can compute the symbol matrix with size $p\times p$ as
...
\end{bmatrix}.
\end{equation}
We refer to $\mathbf{Q}$  as localization operator.

The computation of this symbol matrix extends to more complex PDE with multiple components and in higher dimensions.  Multiple components are supported by extending the $p \times p$ system of Toeplitz operators in \cref{symbolscalar1d} to a $\left( n \cdot p \right) \times \left( n \cdot p \right)$ system of operators, where $n$ is the number of components in the PDE.
The localization operator for a multi-component PDE is given by $\mathbf{Q}_n=\mathbf{I}_n \otimes \mathbf{Q}$, where $I_n$ is the identity matrix with size $n\times n$. In general, we omit the subscript indicating the number of components for the localization operator.
...
\end{equation}
where $ \rho \left( \tilde{\mathbf{S}} \left( \nu, \omega, \boldsymbol{\theta} \right)\right)$ denotes the spectral radius of the matrix symbol $\tilde{\mathbf{S}}$. We would like to comment that  this LFA predicted smoothing factor is based on standard coarsening.
...
If multiple pre or post-smoothing passes are used by our multigrid algorithm, we take the product of the symbol matrix with itself to represent repeated application due to $\rho(AB)=\rho(BA)$ for any square matrices $A$ and $B$.
...
\end{equation}
where $\mathbf{S}_f$ \yh{Why need subscript $f$? Also in the symbol section} represents the smoother error propagation operator and $\mathbf{A}_c^{-1}$ represents the coarse grid solve, which may be another multigrid cycle.
...
LFA in higher dimensions can be given by tensor-products of 1D basis operations, as before.
...
\end{equation}
on the domain $\left[ -3, 3 \right]^3$ with approximately 8 million degrees of freedom for a variety of test cases. \yh{point out boundary condition for your tests}
...
\cref{table:two_grid_3d_chebyshev} shows that LFA predictions match well with the experimental convergence factors.
...
We introduced LFAToolkit.jl \cite{thompson2021toolkit}, a new Julia package for LFA of high-order finite element methods, and used LFAToolkit.jl to investigate p-multigrid method with Jacobi and Chebyshev semi-iterative method smoothing and aggressive coarsening strategies. The LFA framework of p-multigrid presented here can be extended to LFA of h-multigrid method. 
