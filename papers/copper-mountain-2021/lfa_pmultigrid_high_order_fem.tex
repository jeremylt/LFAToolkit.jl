...
Our numerical experiments demonstrating actual convergence rates of p-multigrid were conducted using the libCEED \cite{libceed-joss} with PETSc \cite{petsc-user-ref} p-multigrid example found in the libCEED repository.
...
It is not feasable to tune multigrid individually for multiple complex PDEs with different boundary conditions on a variety of engineering meshes; therefore, we are interested in directly comparing the LFA convergence factors for the problem on an infinite grid with our test problem with Dirichlet boundary conditions.
