...
If the pointwise representation of the weak form $\mathbf{D}$, and therefore the element operator $\mathbf{A}_e$, then the symbol $\tilde{\mathbf{A}}$ will have a tensor product structure, as shown is \cite{he2020two}.
We omit this optimization in LFAToolkit in favor of supporting second-order PDE that do not produce product decomposition.
...
The LFA predicted smoothing factor is given by the maximum spectral radius of the symbol across all frequencies
\begin{equation}
\mu = \max_{\boldsymbol{\theta} \in T} \left\lvert \tilde{\mathbf{S}} \left( \nu, \omega, \boldsymbol{\theta} \right) \right\rvert.
\end{equation}
